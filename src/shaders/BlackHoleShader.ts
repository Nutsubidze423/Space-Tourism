...
void main() {
  ...
  float spiral = theta + r * 4.0 + uTime * 0.3;
  
  vec2 noiseCoord = vec2(spiral * 2.5, r * 6.0);
  float turbulence = fbm(noiseCoord + uTime * 0.08);
  turbulence += fbm(noiseCoord * 2.0 - uTime * 0.1) * 0.4;
  ...
  vec3 coolBlue = vec3(0.1, 0.4, 0.8);
